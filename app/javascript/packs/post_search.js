...
window.search = function search() {
    ...
    const postListingElements = document.getElementsByTagName("li");
    ...
    for (let i = 0 ; i < postListingElements.length ; i++) {
        ...
        if (searchCategoryString.length > 0) {
            ...
            if (searchCategoryString !== categoryElement.innerText.trim()) {
                continue;
            }
        }
...
/** Resets the search form and shows all search results. */
window.clearSearch = function clearSearch() {
    const searchTextElement = document.getElementById("post-search-text");
    const searchCategoryElement = document.getElementById("post-search-category");

    searchTextElement.value = "";
    searchCategoryElement.selectedIndex  = 0;
...
function showAllPostListings() {
    const postListings = document.getElementsByTagName("li");
...
function anyTermsMatch(termsA, termsB) {
    ...
    return termsA.some((searchTerm) => termsB.includes(searchTerm));
}
...
function filterByPostTitle(postListingElement, searchTerms) {
    ...
    const titleTerms = splitStringIntoTerms(titleElement.innerText);
...
function filterByPostDescription(postListingElement, searchTerms) {
    ...
    const descriptionTerms = splitStringIntoTerms(descriptionElement.innerText);
...
function filterByPostTags(postListingElement, searchTerms) {
    ...
    if (tagElements.length > 0) {
        for (let i = 0 ; i < tagElements.length ; i++) {
            const tagTerms = splitStringIntoTerms(tagElements[i].innerText);
...
function filterByPostCategory(postListingElement, searchTerms) {
    ...
    const categoryTerms = splitStringIntoTerms(categoryElement.innerText);
...
function getPostListingCategory(postListingElement) {
    ...
    return (tagElements.length === 0) ? null : tagElements[0];
}
